...
class SqlWrapper {
  ...
  public function getDefeatedGuardians($playerId, $all = false) {
    $readyStr = $all ? "" : " AND ready = 1";
    $guardianNums = $this->game->getObjectListFromDb("SELECT num FROM guardian WHERE in_hand = $playerId".$readyStr." ORDER BY deckorder");
    $boons = [];
